...
import kotlinx.android.synthetic.main.activity_hash4.*
...
class hash4 : AppCompatActivity() {
    ...
    lateinit var hash_back4 : ImageView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_hash4)
        ...
        //뒤로가기 버튼 연결
        hash_back4 = findViewById(R.id.hash_back4)

        //햄버거 버튼으로 드로어 열기
        btnOpen.setOnClickListener {
            email.setText(MySharedPreferences.getUserId(this))
            main_drawer_layout_hash4.openDrawer(GravityCompat.START)
        }
        ...
        //생활비 지원 장학금과 연관된 데이터를 찾아옴. 1로 되어있으면 연관/0으로 되어있으면 연관되지 않음
        var a = "1"
        cursor = sqlitedb.rawQuery("SELECT * FROM august_day WHERE life = '"+a+"';", null)
        ...
        cursor2 = sqlitedb.rawQuery("SELECT * FROM august_day WHERE life = '"+a+"';", null)

        //디데이 계산을 위해 필요 값을 불러옴 (연도,월, 일)
