...
import kotlinx.android.synthetic.main.activity_hash2.btnOpen
...
import kotlinx.android.synthetic.main.activity_hash6.*
...
import java.util.*
import kotlinx.android.synthetic.main.activity_monthjang.menu04 as menu041

class hash6 : AppCompatActivity() {
    ...
    lateinit var hash_back6 : ImageView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_hash6)
        ...
        hash_back6 = findViewById(R.id.hash_back6)

        //햄버거 버튼으로 드로어 열기
        btnOpen.setOnClickListener {
            email.setText(MySharedPreferences.getUserId(this))
            main_drawer_layout_hash6.openDrawer(GravityCompat.START)
        }
        ...
        //장학금 리스트 데이터베이스 파일을 엶
        dbManager = DBManager(this, "august_day", null, 1)
        sqlitedb = dbManager.readableDatabase

        lateinit var cursor : Cursor

        //재학생 장학금과 연관된 데이터를 찾아옴. 1로 되어있으면 연관/0으로 되어있으면 연관되지 않음
        var a = "1"
        cursor = sqlitedb.rawQuery("SELECT * FROM august_day WHERE now = '"+a+"';", null)
        ...
        cursor2 = sqlitedb.rawQuery("SELECT * FROM august_day WHERE now = '"+a+"';", null)

        //디데이 계산을 위해 year,month,day 값을 불러옴
