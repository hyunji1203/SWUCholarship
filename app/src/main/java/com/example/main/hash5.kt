...
import kotlinx.android.synthetic.main.activity_hash5.*
...
class hash5 : AppCompatActivity() {
    ...
    lateinit var hash_back5 : ImageView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_hash5)
        ...
        //뒤로가는 버튼 연결
        hash_back5 = findViewById(R.id.hash_back5)

        //햄버거 버튼으로 드로어 열기
        btnOpen.setOnClickListener {
            email.setText(MySharedPreferences.getUserId(this))
            main_drawer_layout_hash5.openDrawer(GravityCompat.START)
        }
        ...
        //지역 장학금과 연관된 데이터를 찾아옴. 1로 되어있으면 연관/0으로 되어있으면 연관되지 않음
        var a = "1"
        cursor = sqlitedb.rawQuery("SELECT * FROM august_day WHERE rel = '"+a+"';", null)
        ...
        cursor2 = sqlitedb.rawQuery("SELECT * FROM august_day WHERE rel = '"+a+"';", null)
        //디데이 계산을 위해 데이터베이스에서 년,월,일 의 데이터를 가져옴
